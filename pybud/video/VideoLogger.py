...
class VideoLogger:
    ...
    def init_frame_props(self):
        ...
        self.var_sec_width_char = (
                                              self.config.FRAME_WIDTH - self.config.LE_XSTART - 2 * self.config.CONTAINER_PADDING) // self.font_width
        self.var_sec_height_char = (
                                               self.config.FRAME_HEIGHT - self.config.LE_YEND - 2 * self.config.CONTAINER_PADDING) // self.font_height
    ...
    def gen_frame(self):
        ...
        self.frame_drawer.line(
            (self.config.LE_XSTART, self.config.LE_YEND, self.config.FRAME_WIDTH, self.config.LE_YEND),
            fill=self.config.Colors.divider, width=self.config.DIVIDER_WIDTH)
    ...
    def gen_code(self):
        ...
        highlight_start = (float(0.0),
                           float(self.config.CONTAINER_PADDING - self.config.LINE_SPACING + (
        ...
        for adj_line, line in enumerate(self.src):
            self.frame_drawer.text(
                (self.config.CONTAINER_PADDING, self.config.CONTAINER_PADDING + adj_line * self.font_height),
                self.src[adj_line],
